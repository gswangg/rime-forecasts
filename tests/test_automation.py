...
class AutomationTests(unittest.TestCase):
    ...
    def test_polymarket_candidate_filter(self):
        ...
        generic_nba_advance = normalize_market(
            raw_market(question="Will Houston Rockets advance to the Conference Semifinals in the 2026 NBA Playoffs?")
        )
        ok, reason = candidate_filter_reason(generic_nba_advance, now=now)
        self.assertFalse(ok)
        self.assertIn("generic team-match", reason)
        ...
        generic_esports_blowout = normalize_market(
            raw_market(question="Will a team get 13-0'd at BLAST Rivals Fort Worth 2026?")
        )
        ok, reason = candidate_filter_reason(generic_esports_blowout, now=now)
        self.assertFalse(ok)
        self.assertIn("generic team-match", reason)

        generic_tennis_winner = normalize_market(
            raw_market(question="Will Jannik Sinner win the 2026 Men's Singles tournament at the Madrid Open?")
        )
        ok, reason = candidate_filter_reason(generic_tennis_winner, now=now)
        self.assertFalse(ok)
        self.assertIn("generic team-match", reason)
    ...
    def test_kalshi_candidate_filter_and_event_generation(self):
        ...
        multigame_sports = normalize_kalshi_market(
            raw_kalshi_market(
                ticker="KXMVESPORTSMULTIGAMEEXTENDED-S20267F4A142A32A-AA7A350FF95",
                event_ticker="KXMVESPORTSMULTIGAMEEXTENDED-S20267F4A142A32A",
                title="no Over 8.5 runs scored,no Over 7.5 runs scored,yes Boston wins by over 8.5 points",
            )
        )
        ok, reason = kalshi_candidate_filter_reason(multigame_sports, now=now)
        self.assertFalse(ok)
        self.assertIn("multileg sports", reason)
    ...
    def test_price_move_alerts_have_same_watch_cooldown(self):
        with tempfile.TemporaryDirectory() as tmp:
            reasoning = Path(tmp) / "reasoning"
            reasoning.mkdir()
            (reasoning / "example.md").write_text(
            ...
            watch = extract_polymarket_watches(reasoning)[0]
            state = default_state()
            state["last_prices"][watch.slug] = {"price": 0.60, "observed_at": "2026-04-25T00:00:00Z"}
            first_market = normalize_market(raw_market(outcomePrices='["0.66", "0.34"]', bestBid=0.65, bestAsk=0.67))
            ...
            small_followup = normalize_market(raw_market(outcomePrices='["0.72", "0.28"]', bestBid=0.71, bestAsk=0.73))
            suppressed = generate_events(
                markets=[small_followup],
                watches=[watch],
                state=state,
