...
2. **Cadence gate.** If the most recent prediction in `reasoning/` was placed within the last **18 hours** AND no new resolution appeared this cycle AND this drive-prompt.md hasn't been edited since that last prediction, **skip automatically**. The market universe does not refresh on sub-daily cadence and the drive-prompt caller is likely being auto-continued without human review. Journal the skip and move on. This prevents the cycle-9-to-11 rapid-skip pattern from v1. The drive-prompt-edit exception means a methodology change resets the gate — the candidate pool has effectively refreshed even if calendar time hasn't.
