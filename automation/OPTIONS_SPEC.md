...
Status: v0.11 liquidity-drop floor, 2026-05-29.
...
v0.10 auto-deactivates a Situational Awareness thesis fixture when all of its active inner theses have a `paper_closed` ticket and no currently `paper_open` ticket. The deactivation happens at the start of each poll, before fixture iteration, and writes the fixture file with `active: false` and a clear audit note documenting the timestamp and reason. Idempotent. Opt-out per fixture: set `autoDeactivateOnPlannedExit: false`. CLI opt-out: `--no-auto-deactivate`. This closes the noise loop that emitted four NVDA orphan signals after the canonical paper position closed on 2026-05-22.

v0.11 adds a liquidity-drop floor to the `options_thesis_refresh_due` trigger. A `liquidity_drop` reason now fires only when the percentage drop exceeds the threshold AND the resulting liquid-contract count is at or below `--thesis-liquidity-floor-contracts` (default 8). Low-volume small-caps (e.g., CORZ) oscillate +/-50% in liquid-contract count day to day well above the actionable floor; that oscillation is non-actionable noise. CORZ fired three identical `liquidity_drop` refreshes in a week (2026-05-22/27/29) at 10-13 liquid contracts — always tradeable, never threatened. The floor restricts the trigger to genuine tradeability threats.
