...
**Observed:** first the Argentine soccer 1X2 cluster woke and skipped; after volume/spread filters, a high-volume Southampton vs Ipswich draw market woke and skipped for the same reason. Later, a PSG vs Bayern `Both Teams to Score` market woke as the same class. Atlético Madrid vs Arsenal exact-score bins then woke as another generic team-match derivative. A Rockets NBA playoff advancement market also woke, but active sportsbook series odds matched Polymarket almost exactly. A Tampa Bay Lightning NHL Round 2 advancement market later woke as the same generic playoff-series class without an NHL model edge. BLAST Rivals esports MVP/winner/finalist/blowout-prop wakes also required ad hoc team/player/map modeling and produced no clear edge. A Cadillac Championship golf outright woke and matched FanDuel/PGA Tour odds within a couple points. A Madrid Open tennis tournament-winner wake matched the live final odds. A Kalshi cross-category MLB/NBA multileg parlay woke at the product of the individual Kalshi leg asks. Chinese Super League halftime-result markets (`Draw at halftime?`, `<team> leading at halftime?`) later woke as the same generic sports derivative.

**Lesson:** short resolution is not enough. Generic team-vs-team win/draw/BTTS/exact-score/halftime-result/series-advancement/tournament-winner/finalist/outright/blowout-prop and multileg parlay markets should not wake the model unless a future sports/esports/golf/tennis model or cross-venue signal explicitly opts them in.

**Implementation:** Polymarket candidate filtering excludes generic team match questions matching `Will <team> vs. <team> end in a draw?`, `Will <team> win on <date>?`, `<team> vs. <team>: Both Teams to Score`, `<team> vs. <team>: Draw at halftime?`, `<team> leading at halftime?`, `Exact Score: <team> <score> - <score> <team>?`, NBA playoff `Will <team> advance to the Conference Semifinals...` markets, NHL playoff `Will <team> advance to Round N...` markets, common esports tournament-winner/finalist questions like `Will <team> win BLAST ... 20XX?` and `Will <team> make it to the BLAST ... Grand Final?`, esports 13-0 blowout props, generic `Will <player/team> win the 20XX ... Championship?` outright questions, and generic `Will <player/team> win the 20XX ... tournament at the ...?` questions. Kalshi candidate filtering excludes generic `KXMVECROSSCATEGORY-*`, `KXMVESPORTSMULTIGAMEEXTENDED-*`, and comma-joined yes/no multileg sports/parlay markets.
