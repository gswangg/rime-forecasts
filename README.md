...
- [`drive-prompt.md`](./drive-prompt.md) — event-driven operating prompt. Methodology, wake-event handling, daemon operation, stop criteria. Currently v4.5.
...
- [`scripts/sa-thesis-scan.py`](./scripts/sa-thesis-scan.py) — shadow-only Situational Awareness thesis scanner. Reads `options/sa-watchlist.json`, fetches chains via Tradier/fixture provider, emits `options_thesis_review_due` wakes and local candidate artifacts under `automation/state/sa-thesis-candidates/`. No trade/ticket output.
