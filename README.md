...
- **Methodology**: [`drive-prompt.md`](./drive-prompt.md) v4.4. Event-driven market monitoring via `wake-pi`; fast-feedback horizon ladder (1–7d primary, 8–21d secondary, 22–45d tertiary only if high-liquidity), 10pp edge threshold with 3/5+ confidence, moved-market edge discount, Kalshi/Polymarket real-money source policy, CLV checkpoints (+1h/+6h/+24h/close), no-capital shadow participant-signal tracking, and a shadow-only listed-options strategy layer anchored by the Situational Awareness / AI-scaling thesis.
