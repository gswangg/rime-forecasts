...
*As of 2026-08-05 (post-dormancy resolution sweep; repo was dormant 2026-06-16 → 2026-08-05).*

- **Real-money primary score — book complete**: 27 Kalshi/Polymarket-primary predictions placed, **27 resolved, 0 pending**. Real-money resolved Brier: **0.155** vs primary-venue entry Brier **0.300**; beat the entry price on 19/27. Full historical ledger including legacy paper-money Manifold: 36 placed, 35 resolved, total Brier **0.130** vs entry **0.247** (26/35 beat entry). One legacy Manifold prediction (Anthropic 60B ARR) awaits late creator resolution and is drifting toward a large miss (market 79.5% vs my 17%).
- **Daemons**: all stopped as of 2026-08-05. The options daemon and SA thesis scanner ran unattended through the dormancy against inactive/expired fixtures and were shut down during the sweep; Kalshi/Polymarket daemons had been down since 2026-05-19.
- **Methodology**: [`drive-prompt.md`](./drive-prompt.md) v4.10. Event-driven market monitoring via `wake-pi`; fast-feedback horizon ladder (1–7d primary, 8–21d secondary, 22–45d tertiary only if high-liquidity), 10pp edge threshold with 3/5+ confidence, moved-market edge discount, Kalshi/Polymarket real-money source policy, CLV checkpoints (+1h/+6h/+24h/close), no-capital shadow participant-signal tracking, and a shadow-only listed-options strategy/thesis-generation layer anchored by the Situational Awareness / AI-scaling thesis. Live Tradier tape pulls (`scripts/live-tape.py`) are standing practice for every position or thesis evaluation; options daemon wakes carry ATM-straddle implied-move and target-vs-implied ratio so the vol-crush hurdle is checked by default.
...
- [`options/STRATEGY_SITUATIONAL_AWARENESS.md`](./options/STRATEGY_SITUATIONAL_AWARENESS.md) — shadow options strategy note translating Leopold Aschenbrenner's AI-scaling thesis and the Situational Awareness 13F read into monitored option trade lines.
- [`options/sa-watchlist.json`](./options/sa-watchlist.json) — durable watchlist/config for the Situational Awareness thesis-generation scanner.
...
- [`automation/SA_THESIS_SPEC.md`](./automation/SA_THESIS_SPEC.md) — Situational Awareness thesis-generation scanner contract: watchlist schema, triggers, thesis-review wakes, and runtime artifacts.
...
- [`scripts/sa-thesis-scan.py`](./scripts/sa-thesis-scan.py) — shadow-only Situational Awareness thesis scanner. Reads `options/sa-watchlist.json`, fetches chains via Tradier/fixture provider, prequalifies first-seen candidates with provider sanity checks, directional liquidity, and options-structure search, then emits `options_thesis_review_due` wakes and local candidate artifacts under `automation/state/sa-thesis-candidates/`. No trade/ticket output.
- [`scripts/live-tape.py`](./scripts/live-tape.py) — standing-practice live tape helper. Pulls current Tradier quotes for any list of symbols, optional intraday bars, optional thesis-fixture colorization showing gap-to-target. Used before any position, thesis, or direction-call evaluation so judgments are grounded in current market context, not stale wake payloads.
- [`scripts/options-daemon.py`](./scripts/options-daemon.py) — shadow-only options funnel. Reads fixture-defined or provider-backed option theses from `--fixture` or `options/theses/`, applies quote-quality / max-risk / edge gates, prints or emits `options_signal_candidate` wakes, emits `options_thesis_refresh_due` for stale/moved/near-expiry active thesis searches, writes local `execution/options-tickets/` artifacts with `--write-tickets`, and scans open tickets for `options_clv_checkpoint_due` / `options_expiry_or_exit` wakes. No broker/live trading.
- [`scripts/options-chain-fetch.py`](./scripts/options-chain-fetch.py) — fetches/normalizes option-chain snapshots from fixture or Tradier market-data provider (`TRADIER_TOKEN` or `TRADIER_API_KEY` required for Tradier; use `dotenvx run -- ...` for encrypted local env) for thesis fixtures and markouts.
- [`scripts/options-markout.py`](./scripts/options-markout.py) — updates local option ticket artifacts from manual marks or current chain fixtures and prints/appends an `options-ledger.md` row.
