...
- [`scripts/options-daemon.py`](./scripts/options-daemon.py) — shadow-only options funnel. Reads fixture-defined option chains and thesis structures, applies quote-quality / max-risk / edge gates, prints or emits `options_signal_candidate` wakes, and can write local `execution/options-tickets/` artifacts with `--write-tickets`. No broker/live trading.
- [`scripts/options-markout.py`](./scripts/options-markout.py) — updates local option ticket artifacts from manual marks or current chain fixtures and prints an `options-ledger.md` row.
