...
- [`drive-prompt.md`](./drive-prompt.md) — event-driven operating prompt. Methodology, wake-event handling, daemon operation, stop criteria. Currently v4.6.
...
- [`scripts/options-daemon.py`](./scripts/options-daemon.py) — shadow-only options funnel. Reads fixture-defined or provider-backed option theses from `--fixture` or `options/theses/`, applies quote-quality / max-risk / edge gates, prints or emits `options_signal_candidate` wakes, writes local `execution/options-tickets/` artifacts with `--write-tickets`, and scans open tickets for `options_clv_checkpoint_due` / `options_expiry_or_exit` wakes. No broker/live trading.
